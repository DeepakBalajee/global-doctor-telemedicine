...
export async function PATCH(
  ...
  try {
    ...
    logAuditEvent({
      ...
      targetType: 'DOCTOR',
      targetId: doctorId,
      details: `Doctor ${doctorId} verification credentials reviewed and approved.`,
      success: true,
    })

    return NextResponse.json(
      { success: true, message: `Doctor ${doctorId} verified and activated successfully.` },
      { status: 200 }
    )
  } catch (err) {
    console.error('Approve doctor error:', err)
    return NextResponse.json({ error: 'Failed to approve doctor verification.' }, { status: 500 })
  }
}
