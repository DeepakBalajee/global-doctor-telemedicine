...
import { endConsultationSession } from '@/lib/consultations/consultation-store'
...
export async function POST(
  ...
  const result = await endConsultationSession(appointmentId, session.role)

  if (!result.session) {
    return NextResponse.json({ error: result.error || 'Failed to end session.' }, { status: 400 })
  }
