...
import { Stethoscope, CheckCircle2, AlertTriangle, XCircle, Search, ShieldCheck, UserX } from 'lucide-react'
...
export const DoctorVerificationPanel: React.FC = () => {
  ...
  const handleSuspend = async (docId: string) => {
    setMessage(null)
    const res = await suspendDoctor(docId, 'Admin action')
    if (res.success) {
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div className="space-y-1">
          <div className="inline-flex items-center gap-2 rounded-full border border-teal-200 bg-teal-50 px-3 py-1 text-xs font-bold text-teal-800">
            <Stethoscope className="w-3.5 h-3.5 text-teal-600" /> Doctor Credentials & Verification Queue
          </div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight pt-1">
            Doctor Verification & Accounts
          </h1>
          <p className="text-xs text-slate-500">
            Review pending doctor applications, verify licensing credentials, approve applications, or suspend active doctor accounts.
          </p>
        </div>
      ...
      {isLoading ? (
        <div className="min-h-[40vh] flex flex-col items-center justify-center space-y-3">
          ...
          <p className="text-xs font-semibold text-slate-500">Loading doctor directory...</p>
        </div>
        ...
        <Card className="p-10 text-center space-y-3 max-w-md mx-auto">
          ...
          <p className="text-xs text-slate-500">No doctor applications match the selected filter criteria.</p>
        </Card>
      )}
